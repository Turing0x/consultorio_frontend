...
export class UserManagerComponent implements OnInit{
  ...
  public deleteOnePerson( person: Person ): void{
    ...
    swalWithButtoms.fire({
      ...
      if( result.isConfirmed ){
        this.userManager.deleteOnePerson(person.ci).subscribe(
          response => {
            this.person_list = this.person_list.filter( per => per.ci !== person.ci  )
          }
        )
